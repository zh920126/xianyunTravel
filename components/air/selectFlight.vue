  ...
  <div class="selectair">
    <div class="top clearfix">
      ...
      <div class="selectflight">
        <el-select v-model="airport" @change="handleSendAll" style="width:120px" placeholder="起飞机场" size="mini">
          <el-option
        ...
        <el-select v-model="flightDate" @change="handleSendAll" style="width:120px" placeholder="起飞时间" size="mini">
          <el-option
        ...
        <el-select v-model="company" @change="handleSendAll" style="width:120px" placeholder="航空公司" size="mini">
          <el-option
        ...
        <el-select v-model="airSize" @change="handleSendAll" style="width:120px" placeholder="机型" size="mini">
          <el-option
    ...
    <div class="bottom">
      筛选:
      <el-button @click="handleCancle" type="primary" round size="mini">
        撤销
      </el-button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleCancle () {
      ...
      this.airSize = ''
      // 同时发送事件给父组件进行数据的渲染
      const newFlightlist = this.flightselect.flights
    ...
    handleAirport (oldFlightlist) {
      ...
      const newFlightlist = oldFlightlist.filter((value) => {
        if (value.org_airport_name === this.airport) { return true }
      })
    ...
    handleFlightTime (oldFlightlist) {
      ...
      // 获取起飞开始时间
      const before = +this.flightDate.split(',')[0]
      // console.log(before)
      // 获取起飞结束时间
      const after = +this.flightDate.split(',')[1]
      // console.log(after)
      // 进行起飞时间的筛选
      const newFlightlist = oldFlightlist.filter((value) => {
        if (+value.dep_time.split(':')[0] >= before && +value.dep_time.split(':')[0] < after) { return true }
      })
      // console.log(newFlightlist);
      // this.$emit("filterMessage",newFlightlist)
      return newFlightlist
    ...
    handleAirSize (oldFlightlist) {
      ...
      // 筛选机型
      const newFlightlist = oldFlightlist.filter(value => value.plane_size === this.airSize)
      // console.log(newFlightlist)
      // this.$emit('filterMessage',newFlightlist)
      return newFlightlist
    },
